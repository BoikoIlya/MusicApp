...
interface SelectedTracksInteractor {
    ...
    suspend fun map(sortingState: SortingState, playlistId: Int): List<SelectedTrackUi>
    ...
    class Base @Inject constructor(
        ...
        private val repository: CachedTracksRepository<SelectedTrackDomain>,
        ...
        override suspend fun isDbEmpty(): Boolean = repository.isDbEmpty(mainPlaylistId)
        ...
        override suspend fun map(sortingState: SortingState, playlistId: Int): List<SelectedTrackUi> {
            val selectedItems = store.read()
            val tracks = repository.fetch(sortingState, playlistId).first()
            var selectedTrackBackgroundColor =  0
            var selectedTrackIconVisibility =  0
            return  tracks.map {media->
               ...
               val mapper = SelectedTrackDomain.ToUiSelected(selectedTrackIconVisibility,selectedTrackBackgroundColor)
                media.map(mapper)
            }
        }
